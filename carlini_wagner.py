...
class attack_orth(MinimizationAttack):
...
class CarliniWagner(fa.L2CarliniWagnerAttack):
    def run(
        ...
        ** kwargs: Any,
        ...
        if len(dirs):
            idx = np.argmin(losses[best_binary_search_step, 0])

            # plt.figure()
            # plt.plot(diffs[best_binary_search_step][:idx])
            # plt.title('Gram-Schmidt difference over itarations')

            fig, ax = plt.subplots(2, 2, squeeze=False)
            fig.subplots_adjust(hspace=0.5)
            plt.suptitle('Loss functions for orth_const = ' + str(orth_const))
            ax[0, 0].set_title('loss')
            ax[0, 0].plot(range(self.steps)[:idx], losses[best_binary_search_step, 0, :idx])
            ax[0, 1].set_title('is_adversarial_loss')
            ax[0, 1].plot(range(self.steps)[:idx], losses[best_binary_search_step, 1, :idx])
            ax[1, 0].set_title('squared_norms')
            ax[1, 0].plot(range(self.steps)[:idx], losses[best_binary_search_step, 2, :idx])
            ax[1, 1].set_title('is_orth')
            ax[1, 1].plot(range(self.steps)[:idx], losses[best_binary_search_step, 3, :idx])
...
#                 gram_schmidt = ep.zeros_like(s)
#                 for i,a in enumerate(adv):
#                     gram_schmidt.raw[i] = (dirs.float32()[i].matmul(s[i].reshape([dirs.shape[-1], 1])) * dirs[i]).sum(axis=0).reshape(s[i].raw.shape).raw
#                 adv_orth=adv-gram_schmidt
#                 logits = model(adv_orth)
#                 # orth_const = np.maximum(min_orth,min_orth * gram_schmidt.abs().sum().item())
#                 adv = ep.zeros_like(x_attack)
...
#             ######## by David ############
#             loss = is_adv_loss.sum() + squared_norms.sum() + is_orth.sum()
#             losses[binary_search_step,:,step] = np.array([loss.item(),is_adv_loss.sum().item(),squared_norms.sum().item(),is_orth.sum().item()])
#             ###############################
#
#             return loss, (adv, logits)
#
