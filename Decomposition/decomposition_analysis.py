...
data = np.load('../data/madry_l2.npy', allow_pickle=True).item()
...
#     ax[j, 0].set_xticks([])
#     ax[j, 0].set_yticks([])
#     ax[j, 0].set_xlabel(str(labels[j]), fontdict={'fontsize': 15})
#
#     for i, a in enumerate(advs[j,:5]):
#         if j==0:
#             ax[j, i+1].set_title('Adversarial ' + str(j + 1))
#         ax[j, i+1].set_xlabel('\u279E ' + str(int(classes[j,i])), fontdict={'fontsize': 15})
...
# for i in unique_idx:
#     dec_space.save_dec_movie(images[i], advs[i,0], advs[i,1], model_natural, n=200, k=i)
