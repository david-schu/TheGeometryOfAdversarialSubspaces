...
def save_dec_movie(orig, adv1, adv2, model, n=100, k=0):
    matplotlib.use('Agg')
    ...
    for i in range(n+2):
        ...
        plt.subplot(2, 1, 1)
        pl.plot_dec_space(scaled_orig, dir1+np.reshape(scaled_orig, (784)), dir2+np.reshape(scaled_orig, (784)), model,
        ...
        lg = f.legend(handles=labels, title='predicted class', loc='upper right', bbox_to_anchor=(0.95, 0.85), prop={'size': 14})
...
def get_mean_dist_dec(orig, adv1, adv2, model):
    dir1 = adv1 - orig.reshape(784)
    dir2 = adv2 - orig.reshape(784)
    ...
    scales = np.linspace(0, 2, 500).reshape((-1,1))
